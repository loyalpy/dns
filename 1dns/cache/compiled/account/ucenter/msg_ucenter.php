...
<div class="mainnav <?php if(in_array($inc,array("order","finance","ucenter"))){?>mainnav-<?php }?>" id="MainNav">
  ...
  <li><a href="<?php echo U("account@/ucenter/safety_center");?>" <?php if(in_array($inc,array("ucenter"))){?>class="cur"<?php }?>><i class="am-icon-gear"></i> &nbsp;设置</a></li>
	    ...
	    <div class="uc-msg msg-<?php echo isset($type)?$type:"";?>">
	    	   <div class="dis60"></div>
	     	   <!--提示信息-->
			   <div class="uc-msg-box">			      
			      <div class="title">
			      <cite class="icon"></cite>
			      <span class="<?php if($type == 'success'){?>font-green<?php }else{?>font-org<?php }?> f18 txt"><?php echo isset($title)?$title:"";?></span>
			      </div>
			      <?php if($info){?>
			      <div class="dis15"></div>
			      <div class="info"><font class="font-gray f12"><?php echo isset($info)?$info:"";?></font></div>
			      <?php }?>
			   </div>
			   <div class="cl"></div>
			   <?php if($btninfo){?>
			   ...
			   <div class="uc-msg-btn">
				...
				<a href="<?php echo isset($tmp[0])?$tmp[0]:"";?>" class="am-btn <?php if($key == 0){?><?php if($type == 'success'){?>am-btn-success<?php }else{?>am-btn-danger<?php }?><?php }?>"><?php echo isset($tmp[1])?$tmp[1]:"";?></a>
				<?php if($key == 0){?>
				<script language="javascript">
					setTimeout(function(){
						$.redirect("<?php echo isset($tmp[0])?$tmp[0]:"";?>");
					},3000);
				</script>
				<?php }?>
				<?php }?>
