...
<script type="text/template" id="tpl_addedit">
    <#macro rowedit data>
                ...
                <div class="tselect-box" style="display:none">
                    <div class="in">
                        ...
                        <?php if($serverR['data']['diyLine']['value'] == 1){?>
                            <?php $diyIp = M("domain_acl_set")->query("domain='$domain' AND uid=$uid","name,id,uid,status","id DESC",500);?>
                            <?php if(count($diyIp)>0){?>
                                <hr/>
                                <div class="level1"><a class="sel0" style="color:darkgray;">自定义线路</a></div>
                                <?php foreach($diyIp as $key => $item){?>
                                <div class="level0"><a class="sel<?php if($item['status'] == 2){?>1<?php }else{?>0<?php }?>" <?php if($item['status'] == 2){?>title='线路已锁定'<?php }?> data-v="cust<?php echo isset($item['id'])?$item['id']:"";?>" href="javascript:void(0);"><?php echo isset($item['name'])?$item['name']:"";?></a></div>
                                <?php }?>
                            <?php }?>
                        <?php }?>
                    </div>
     ...
     <#else>
             ...
             <td><span class="btn-edit">${data.RRname}</span><#if (data.inlock == 1)><font class="am-text-danger" title="解析已锁定，待审核" style="cursor: pointer">&nbsp;&nbsp;<i class="am-icon-lock"></i></font></#if></td>
             ...
             <td class="am-text-right">
                 <?php if(!$domain_bind){?>
                     <#if (data.status == 0)><i class="am-icon-pause" title="已暂停解析"></i>&nbsp;</#if>
                     <a class="moniter_bind" href="<#if (data.monitor.length == 0)><?php echo U("/monitor/monitor_option_record?domain=");?>${data.domain}&RRname=${data.RRname}<#else><?php echo U("/monitor/monitor_detail?do=detail&RRname=");?>${data.RRname}&domain=${data.domain}&record_id=${data.record_id}</#if>"  title="<#if (data.monitor.length == 0)>该记录可以开启监控功能，立刻开启<#else><#if (data.monitor.status==1)><#if (data.monitor.status_code == 200)>您的网站一切正常，点击查看详情<#else>您的网站出了点问题，点击查看详情</#if><#else>您的网站已宕机，点击查看详情</#if></#if>"><i class="am-icon-desktop <#if (data.monitor.length == 0)>
                     am-text-info
                     <#else>
                        <#if (data.monitor.status==1)>
                            <#if (data.monitor.status_code!=200)>am-text-warning
                            <#else>am-text-success
                            </#if>
                        <#else>am-text-danger
                        </#if>
                    </#if>"></i></a>&nbsp;&nbsp;
                     <i class="am-icon-edit btn-edit" title="编辑"></i>
...
<script type="text/javascript">
    ...
    var domain_bind = "<?php echo isset($domain_bind)?$domain_bind:"";?>";
    ...
    var add_edit_record = function(cur_row){
        //判断如果是绑定域名，不能进行一系列操作
        if (!$.is_empty(domain_bind)) {
            $.ui.error("域名已被其它域名绑定，请解绑后操作！");
            return false;
        }
        var formhtml = "";
        if($.is_empty(cur_row)){     //添加
            var data = {domain_id:<?php echo isset($domain_id)?$domain_id:"";?>,record_id:0,'type':'edit',RRtype:'A',RRname:'',RRvalue:'',acl:'any',RRmx:'',RRttl:600,status:1};
            ...
            switch(RRtype){
                ...
                case 'URLN':
                    ...
                    if(!$.dns.is_request_url(RRvalue)){
                        $.ui.tooltip("记录值不正确,格式：http://www.bajiedns.com！",new_cur_row.find(".RRvalue").get(0));
                        return false;
                    }
                    break;
            ...
            $.ajaxPassport({
                ...
                success: function(res){
                    if(res.status == 1){
                        ...
                        show_hide_tips(true);
                        //2016.11.21 update by py
                        $.exeJS("<?php echo U("/domains/refresh?domain=$domain");?>");
                        //根据频率执行域名监控
                        if (res.data.monitor_rate) {
                            $.exeJS("<?php echo U("/monitor/update_rate_queue?rate=");?>"+res.data.monitor_rate);
                        }
                    }else{
...
<script type="text/javascript">
    var curpage = 1;
    var get_records_list = function(page,keyword,name,value,type,status,acl){
        var keyword    = $.is_empty(keyword)?'':keyword;
        var name        = $.is_empty(name)?'':name;
        var value         = $.is_empty(value)?'':value;
        var type          = $.is_empty(type)?'':type;
        var status        = $.is_empty(status)?'':status;
        var acl             = $.is_empty(acl)?'':acl;
        $.ui.loading($(".record-list"));
        $.ajaxPassport({
            url:U("/api/DomainRecord.GetListByDomain"),
            success: function(res){
...
//                    $('.list_tpl').append("<tr><td colspan='8'><a href='##' class='am-icon-exclamation-circle am-text-danger am-text-lg'></a> <a href='##' class='am-font-gray'>未找到记录结果?</a></td></tr>");
//                    $(".list_tpl_m").hide();
                    $('.pagebar').html("");
                }else{
            ...
            data:{domain:"<?php echo isset($domain)?$domain:"";?>",page:page,keyword:keyword,RRname:name,RRvalue:value,RRtype:type,status:status,acl:acl}
        });
    }
    var batch_op_records = {
        url: U("/api/DomainRecord."),
        exe_url:"",
        data:{},
    };
    var ids= new Array();
    var batch_records_op = function(obj) {
        ...
        if (record_do == "stop") {
            ...
            batch_op_records.exe_url =batch_op_records.url + "Status";
            batch_op_records.data.status = 0;
        } else if (record_do == "start") {
            record_confirm = "你确定要启用记录吗？";
            batch_op_records.exe_url =batch_op_records.url + "Status";
            batch_op_records.data.status = 1;
        }else if (record_do == "del") {
            record_confirm = "你确定要删除记录吗？";
            batch_op_records.exe_url =batch_op_records.url + "Del";
        }
        $.ui.confirm(function(){
    ...
    var batch_records_detial_op = function(i){
        batch_op_records.data.record_id=ids[i];
        ...
        $.ui.success("操作中..");
        $.ajaxPassport({
            url: batch_op_records.exe_url,
            success: function (res) {
                if (res.status == 1) {
                    num++;
                    var recordId = ".record_"+ids[i];
                    if (batch_op_records.exe_url.split(".")[1] == 'Del') {
                        ...
                        if(batch_op_records.data.status==1){
        ...
        <div class="am-modal am-modal-no-btn" tabindex="-1" id="doc-modal-1">
            <div class="am-modal-dialog">
                <div class="am-modal-hd" style="border-bottom: 1px solid silver;text-align: left;color:black;padding-bottom: 16px;">
                    <i class="am-icon-cloud-upload" style="color: #EB8500;"></i>&nbsp;&nbsp;批量导入记录&nbsp;&nbsp;<span style="font-size: 12px;color: grey">小贴士：请选择txt格式的文件！</span>
                    <a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close>&times;</a>
                </div>
                <div class="dis10"></div>
                <div class="am-modal-bd">
                    <div class="dis20"></div>
                    <table style="margin-left: 80px;line-height: 60px;">
                        <col width="100px"/>
                        <col width="200px"/>
                        <tbody>
                        <tr style="text-align: left">
                            <td style="font-weight: 700">记录域名:</td>
                            <td><?php echo isset($domain)?$domain:"";?></td>
                        </tr>
                        <tr style="text-align: left">
                            <td style="position:relative;top:-45px;font-weight: 700">
                                <span>TXT文件:</span><br/>
                                <span>导入模式:</span>
                            </td>
                            <td>
                                <form action="<?php echo U("account@/records/Import?do=import");?>"  enctype="multipart/form-data" method="post" class="theform form form-2" role="form">
                                    <div class="form-content">
                                            <input type="file" name="ipdatafile" value="" class="am-btn am-btn-default am-btn-xs"/>
                                    </div>
                                    <div class="am-form-group" style="margin-top: 12px;">
                                        <?php $import_type = tCache::read("data_config")?>
                                        <?php if(isset($import_type['record_import_type'])){?>
                                        <?php foreach($import_type['record_import_type'] as $key => $item){?>
                                        <label class="am-radio-inline">
                                            <input type="radio"  value="<?php echo isset($key)?$key:"";?>" name="moshi" <?php if($key == 2){?>checked<?php }?>> <?php echo isset($item)?$item:"";?>
                                        </label>
                                        <?php }?>
                                        <?php }else{?>
                                        <label class="am-radio-inline">
                                            <input type="radio" name="moshi" value="2" checked> 八戒DNS
                                        </label>
                                        <?php }?>

                                    </div>
                                    <input type="hidden" name="domain" value="<?php echo isset($domain)?$domain:"";?>"/>
                                    <button type="submit" class="am-btn am-btn-warning domain-records-import">立即导入</button>&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    var batch_import_records = function () {
        var html = "" + easyTemplate($("#tpl-setting-import").html());
        $(".tpl_setting_import").html(html);
        $(".tpl_setting_import").find('#doc-modal-1').modal({width: 600});

        $.ajaxPassport({
            url:"<?php echo U("account@/records/Import");?>",
            success:function(res){
                if(res.error == 1){
...
//                            $.ui.loading();
                            curform.find("button[type='submit'],input[type='submit']").html( "<i class='am-icon-spinner am-icon-spin'></i>正在导入...");
                            var thebtn = curform.find("button[type='submit'],input[type='submit']").get(0);
                            curform.data("success",function(res,that){
//                                $.ui.close_loading();
                                if(res.error == 0){
                                    $.ui.success(res.message);
                                }else{
                                    $.ui.error(res.message);
                                    curform.find("button[type='submit'],input[type='submit']").html("立即导入");
                                    return false;
                                }
