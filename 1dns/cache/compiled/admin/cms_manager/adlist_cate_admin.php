...
<div class="menu">
 <ul>
	 <?php if($this->check_purview('/cms_manager/adlist')){?>
 <li><a href="<?php echo U("/cms_manager/adlist");?>" class="<?php if('/'.$inc.'/'.$act == '/cms_manager/adlist'){?>cur<?php }?>">广告列表</a></li>
	 <?php }?>
	 <?php if($this->check_purview('/cms_manager/adlist_cate')){?>
...
<div class="main-nav">
  ...
  <button type="button" data-url="/sys_manager/data_config?do=makecache" class="btn btn-default btn-sm t-ajax-button" data-loading-text="处理中……">&nbsp;<cite class="glyphicon glyphicon-refresh"></cite>&nbsp;</button>
	  <?php if($this->check_purview('/cms_manager/adlist_cate_edit')){?>
	  <a href="javascript:void(0)" data-url="<?php echo U("/cms_manager/adlist_cate?do=edit&id=0");?>" class="btn btn-primary btn-sm addbtn">
...
<#macro row data>
	...
	<p class="table-item-op">
		<?php if($this->check_purview('/cms_manager/adlist_cate_edit')){?>
	<a href="javascript:void(0);" class="editbtn" data-url="<?php echo U("/cms_manager/adlist_cate?do=edit&id=");?>${data.code}"><span class="glyphicon glyphicon-edit"></span></a>
		<?php }?>
		<?php if($this->check_purview('/cms_manager/adlist_cate_refresh')){?>
		<a href="javascript:void(0);" class="t-ajax-button" data-url="<?php echo U("/cms_manager/adlist_cate?do=refresh&id=");?>${data.code}"><span class="glyphicon glyphicon-repeat"></span></a>
		<?php }?>
		<?php if($this->check_purview('/cms_manager/adlist_cate_del')){?>
		<a href="javascript:void(0);" class="delbtn" data-url="<?php echo U("/cms_manager/adlist_cate?do=del&id=");?>${data.code}"><span class="glyphicon glyphicon-remove"></span></a>
		<?php }?>
	</p>
	</td>
	</tr>
...
<form action="<?php echo U("/cms_manager/adlist_cate?do=edit");?>" method="post" class="theform form form-1" role="form">
<div class="tpl"></div>
</form>
</div>
</#macro>
...
var load_list = function(){
	...
	},"<?php echo U("/cms_manager/adlist_cate?do=get");?>");
}
var edit_func = function(obj){
	var edit_c = $("#tpl_list_edit").html();
	var url = $(obj).attr("data-url");
	edit_c = "" + easyTemplate(edit_c,{title:"添加/修改广告分类"});
