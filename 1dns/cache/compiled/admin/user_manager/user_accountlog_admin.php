...
<div class="main-nav">
  ...
  <div class="navbtn0">
  <?php $ftype_s = array("balance"=>"账户余额","sms"=>"短信","point"=>"积分")?>
  <?php foreach($ftype_s as $key => $item){?>
  <a class="btn <?php if($ftype == $key){?>btn-info<?php }else{?>btn-default<?php }?> btn-sm" title="<?php echo isset($item)?$item:"";?>" href="<?php echo U("/user_manager/user_accountlog?ftype=$key");?>"><cite class="glyphicon glyphicon-th"></cite> <?php echo isset($item)?$item:"";?></a> 
  <?php }?>
  </div>
  <div class="navbtn">
   <!--button-->
	  <?php if($this->check_purview("/user_manager/user_accountlog")){?>
...
<!--search box-->
<form class="the_searchform form" method="POST" action="<?php echo U("/user_manager/user_accountlog?do=get_url");?>">
<div class="tpl"></div>
</form>
<!--end search box -->
<!-- list box -->
...
var search_formdata = {
	...
	ftype:{type:"hidden",label:"",name:"ftype",value:"<?php echo isset($ftype)?$ftype:"";?>",data_sr:[],css:"",require:"",desc:"",item_css:"item-200"},
