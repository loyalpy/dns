...
<div class="am-uc-right">
    <div class="am-alert am-alert-danger basic-t-t" data-am-alert>
        ...
        <p>温馨提示：<?php echo isset($register_type)?$register_type:"";?>域名需及时提交实名制审核资料，实名制审核不通过将禁止解析。</p>
    </div>
    <div>
    ...
    <div class="am-g">
        <div class="am-btn-group">
            <a href="<?php echo U("/ucenter/index");?>" type="button" class="am-btn am-btn-success am-btn-sm am-radius"><i class="am-icon-align-justify"></i> 服务期域名</a>
            <a href="<?php echo U("/ucenter/index_need");?>" type="button" class="am-btn  am-btn-default am-btn-sm am-radius"><i class="am-icon-align-center"></i> 续费期域名</a>
            ...
            <#if (data.list.length > 0)>
                <#list data.list as domain>
                        <tr>
                            ...
                            <td class="d"><span><a href="<?php echo U("/ucenter/basic?domain=");?>${domain.domain}">${domain.domain}</a></span></td>
                            <td>${domain.domain_name}</td>
                            <td class="am-font-gray"><#if (domain.type == 0)>国际域名<#else>国内域名</#if></td>
                            <td>${domain.reg_time}</td>
                            <td>${domain.exp_time}</td>
                            <td><span style="color: #5EB95E">服务期</span></td>
                            <td class="c">
                                <a type="button" href="javascript:;" class="am-ext-btn domain-renew" data-domain_id="${domain.id}">续费</a>&nbsp;
                                <a type="button" class="am-ext-btn <#if (domain.is_ns == 0)>00perse</#if>" data-domain="${domain.domain}" href="<?php echo U("account@/domains/dns/");?>${domain.domain}" target="_blank">解析</a>&nbsp;
                                <a type="button" class="am-ext-btn" href="<?php echo U("/ucenter/basic?domain=");?>${domain.domain}">管理</a>
                            </td>
                        </tr>
               </#list>
                <#else>
                    <tr>
                        <td class="am-text-sm" colspan="8">
                            <a href="##" class="am-icon-exclamation-circle am-text-danger am-text-lg"></a> <a href="##" class="am-font-gray">未找到域名?</a> &nbsp;<a href="<?php echo U("domain@/domain/index");?>" target="_blank">去注册</a></td>
                    </tr>
            </#if>
            </tbody>
        </table>
        ...
        <div class="am-modal am-modal-no-btn" tabindex="-1" id="doc-modal-1">
            <div class="am-modal-dialog">
                <div class="am-modal-hd" style="border-bottom: 1px solid silver;text-align: left;color:black;padding-bottom: 16px;">
                    <i class="am-icon-share-square-o" style="color: #EB8500;"></i>&nbsp;&nbsp;温馨提示&nbsp;&nbsp;
                    <a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close>&times;</a>
                </div>
                <div class="dis10"></div>
                <div class="am-modal-bd" style="font-size: 14px;text-align: left;padding: 30px 30px 60px 30px;text-align: center;">
                    <div style="font-size: 16px">您的域名没有指向八戒DNS，暂不能使用此功能！</div>
                    <div class="dis30"></div>
                    <a href="<?php echo U("/ucenter/basic_dns?domain=");?>${domain.domain}" type="button" class="am-btn am-btn-warning">修改DNS</a>&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    var load_domains_list = function(page,keyword,orderby){
        ...
        $.ajaxPassport({
            ...
            success:function(res){
                ...
                //域名dns不在我们这边
                $(".perse").unbind("click").bind("click",function () {
                    var domain = $(this).data("domain");
                    res = {domain:domain};
                    var html = "" + easyTemplate($("#tpl_domain_find").html(),res);
                    $(".my-domian-perse").html(html);
                    $(".my-domian-perse").find('#doc-modal-1').modal({width: 450});
                    return false;
                });
                //域名续费
                ...
                $(".domain-register-table").find("a.orderby").each(function () {
                    ...
                    $(obj).unbind("click").bind("click", function () {
                        load_domains_list(1, keyword,(or_item + "!" + (or_v == "ASC" ? "DESC" : "ASC")));
                    });
                });
            },
            data:{page:page,keyword:keyword,orderby:orderby},
        });
    }
