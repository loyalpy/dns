		...
		<div class="sidebar">
			<div class="artarea">
				<?php if($data['list']){?>
					...
					<dl>
						<dt><a href="<?php echo U("/cms/topic/tid/$item[tid]");?>" target="_blank"><?php echo tFun::keyword_replace($keyword,$item['subject']);?></a></dt>
						<dd class="art"><a href="<?php echo U("/cms/topic/tid/$item[tid]");?>" target="_blank"><?php echo tFun::keyword_replace($keyword,tUtil::substr($item['description'],120,1));?>..</a></dd>
						<dd class="time">
							<!--<span><?php echo tTime::get_datetime("Y-m-d H:i",$item['dateline']);?></span>-->
							<div class="s">分享<img src="<?php echo U("")."skins/".$this->app."/".$this->skin."/images/index/upp.jpg";?>"></div>
							<a class="more" href="<?php echo U("/cms/topic/tid/$item[tid]");?>">查看详情</a>
							<ul class="share" style="display: none;">
								<li style="position: relative;top:-3px;">分享到：</li>
					...
					<?php }?>
				<?php }else{?>
				<div class="f14 font-red"><img src="<?php echo U("")."skins/".$this->app."/".$this->skin."/images/noresult.png";?>" />  <span style="position:relative;top:-20px;">很抱歉，暂无信息</span></div>
				<?php }?>
			</div>
			<div class="pagebar">
...
<script type="text/javascript">
	$(function(){
		...
		$(".s").unbind("click").bind("click",function(){
			var obj = this;
			if($(obj).parent().find(".share").css('display') == 'none'){
				$(obj).parent().find(".share").show();
			}else{
				$(obj).parent().find(".share").hide();
			}
		})
