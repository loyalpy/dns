...
<div class="dns-tools">
    <div class="content">
        ...
        <div class="tools-top">
            <p class="p1">DNS查询工具</p>
            <p class="p2">此工具可以协助您查询域名DNS相关问题</p>
        </div>
        ...
        <div class="row">
            <form class="the_searchform form" method="POST" action="<?php echo U("/tools/dnsquery");?>" onsubmit="return loadresult();">
                <div class="row-content c1">
                    <span>请输入域名:</span><input type="text" class="text" name="question" value="<?php echo isset($this->question)?$this->question:"";?>"/><span class="submit">提交查询</span><br/>

                    <input type="hidden" name="hash" value="<?php echo tUtil::hash();?>" />
                    <button type="submit" style="display: none"></button>

                    <span class="dns-span">DNS服务器:</span><input type="text" class="text" name="server" value="<?php echo isset($this->server)?$this->server:"";?>"/>
                    <input type="checkbox" name="extendanswer" value="1" <?php if($this->detail == 1){?> checked="checked"<?php }?>/> 显示细节<br/>
                    <input type="radio" <?php if($this->type == 'A'){?> checked="checked" <?php }?> name="type" value="A"/> 主机服务记录(A)&nbsp;&nbsp;
                    <input type="radio" <?php if($this->type == 'CNAME'){?> checked="checked" <?php }?> name="type" value="CNAME"/> 别名记录(CNAME)&nbsp;&nbsp;
                    <input type="radio" <?php if($this->type == 'NS'){?> checked="checked" <?php }?> name="type" value="NS"/> 名称服务记录(NS)&nbsp;&nbsp;
                    <input type="radio" <?php if($this->type == 'MX'){?> checked="checked" <?php }?> name="type" value=""/> 邮件交换记录(MX)
                </div>
            </form>
            <div class="dis10"></div>
            ...
            <div class="result">
                <?php if($this->result){?>
                    <p>查询结果</p>
                    <div class="query-res" style="color: #F74F06">
