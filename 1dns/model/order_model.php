<?php
class order_model extends tModel{
	...
	public function get($order_no = ""){
		$return = array();
		if($order_no){
			$return = M("order")->get_row("order_no='{$order_no}'");
	...
	public function get_item($order_no = ""){
		$return = array();
		if($order_no){
			$return = M("order_item")->query("order_no='{$order_no}'","*","type ASC");
		}
		return $return;
	}
	public function get_list($data = array(), $pageurl = "")
	{
		$data['table'] = isset($data['table']) ? $data['table'] : "order";
		...
		foreach($list['list'] as $key =>$v){
			...
			if ((time() - $v['dateline']) > 86400*3) {
				if (in_array($v['status'],array(1,2))) {
					if (M("order")->set_data(array("indel"=>1))->update("order_no = '{$v['order_no']}'")) {
						M("order_item")->set_data(array("indel"=>1))->update("order_no = '{$v['order_no']}'");
						unset($list['list'][$key]);
					}
				}
			}
