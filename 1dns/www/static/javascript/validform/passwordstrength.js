	...
	$.fn.passwordStrength=function(settings){
		...
		this.each(function(){
			...
			$this.bind("keyup blur",function(){
				...
				pstrength.find("span").removeClass("c0 c1 c2");
				if(scores < 35 && scores >=0){
					pstrength.find("span:first").addClass("c0");
				}else if(scores < 60 && scores >=35){
					pstrength.find("span:eq(1)").addClass("c1");
					//pstrength.find("span:lt(2)").addClass("c1");
				}else if(scores >= 60){
					pstrength.find("span:eq(2)").addClass("c2");
					//pstrength.find("span:lt(3)").addClass("c2");
				}
				...
				if(checkingerror && ($this.val().length<settings.minLen || $this.val().length>settings.maxLen) ){
					...
					settings.showmsg($this,$this.attr("sucmsg"),2);
				}
