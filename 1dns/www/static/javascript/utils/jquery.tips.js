...
(function($){
    $.tooltipsy = function (el, options) {
        ...
        //如果已经存在则摧毁
        if(this.$el.data("tooltipsy")){
            this.$el.data("tooltipsy").destroy();
        }
    ...
    $.tooltipsy.prototype = {
        init: function () {
            ...
            }else if(settings.showEvent === "none"){
            	base.show();
                ...
                var tip_position = [
                    (function () {
                        if (settings.offset[0] < 0) {
                            ...
                            return offset.left;//offset.left - ((base.width - $el.outerWidth()) / 2);
                        }
                        else {
        ...
        readify: function () {
            this.ready = true;
            this.$tipsy = $('<div class="tooltipsy" id="tooltipsy' + this.random + '" style="position:fixed;z-index:2147483647;display:none">').appendTo('body');
            this.$tip = $('<div class="' + this.settings.className + '">').appendTo(this.$tipsy);
            this.$tip.data('rootel', this.$el);
            var e = this.$el;
            var t = this.$tip;
            var c = this.settings.content != '' ? (typeof this.settings.content == 'string' ? this.settings.content : this.settings.content(e, t)) : this.title;
            this.$tip.html(c+"<span class=\"other\"></span>");
        },
...
/* ext */
$.ajaxTips = function(objstr,msg,type,offset,delay){
	var _type = typeof type == "undefined"?"success":type;
	var _offset = typeof offset == "undefined"?[0,-1]:offset;
	var _delay = (_type == "loading"?9999999:(delay?delay:3000));
	var _disabled = (_type == "loading"?"disabled":false);
	$(objstr).attr("disabled",_disabled);
	if(_type == "hide"){
		$(objstr).data('tooltipsy').hide();
	}else{
		var debug = 1;
		if(_type == "error" && debug == 1){
			$("body").append("<div class='ajax_error_iframe'>"+msg+"</div>");
		}else{
			$(objstr).tooltipsy({
				content: msg,
				className:_type,
