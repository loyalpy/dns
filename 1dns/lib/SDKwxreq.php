...
class SDKwxreq{
    ...
    public static function switchType(&$request){
        \tCache::write("@weixin-test-test",$request);
        $data = array();
    ...
    public static function text(&$request){
        ...
        //\tSlog::log($content.$request['fromusername']);
        if($content == "八戒DNS"){
           $content = '非常感谢您关注我们';
           //"http://static1.1dns.com/attach/avatar/000/00/00/weixin.jpg"
           return ResponsePassive::text($request['fromusername'], $request['tousername'], $content); 
        }else{
           $content = '试试输入 "八戒DNS" 看看';
           return ResponsePassive::text($request['fromusername'], $request['tousername'], $content); 
        }
    ...
    public static function voice(&$request){
        if(!isset($request['recognition'])){
            ...
            $content = '您刚才是说"'.$request['recognition'].'" 么?';
            return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
        }
    }
    ...
    public static function eventQrsceneSubscribe(&$request){
        ...
        */
        ...
        if(!empty($sceneid) && $openid){
            $uid    = intval($sceneid);
            $data   = \SDKwx::get_userinfo($openid);
            $res = M("@user_bd")->wxbd($uid,$openid,$data);

            //方便开发人员调试时查看参数正确性
            if(is_int($res)){
                if($res){
                    $res_userinfo = C("user")->get_cache_userinfo($res);
                     $content = '该微信号已经绑定账号"'.$res_userinfo['name'].'",请将原账号解绑后再绑定';
                }else{
                    $content = "绑定失败";
                }                
            }else{
                $content = '欢迎您关注我们的微信，帐号已成功绑定我们官方微信';
            }            
            return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
        }
        return "";
    ...
    public static function eventClick(&$request){
        ...
        switch ($eventKey) {
            case "LOCK_ACCOUNT":
                ...
                if (isset($row['uid'])) {
                    $userinfo = C("user")->get_cache_userinfo($row['uid']);
                    if ($userinfo['uid']) {
                        if ($userinfo['inlock'] == 0) {
                            $data = array(
                                "inlock" =>1,
                            );
                            C("user")->update_user($row['uid'],$data);
                            $content = "已经锁定你的账号{$userinfo['email']}";
                        }elseif ($userinfo['inlock'] == 2) {
                            $content = "操作失败，你的账号{$userinfo['email']}已被系统锁定";
                        }else{
                            $content = "你的账号{$userinfo['email']}已经锁定";
                        }
                    }else{
                        $content = "账号未找到";
                    }
                }else{
                    $content = "未绑定八戒DNS系统账号";
                }
                return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
                break;
            case "UNLOCK_ACCOUNT":
                ...
                if (isset($row['uid'])) {
                    ...
                    if ($userinfo['uid']) {
                        if ($userinfo['inlock'] == 1) {
                            ...
                            C("user")->update_user($row['uid'],$data);
                            $content = "你的账号{$userinfo['email']}已解锁";
                        }elseif ($userinfo['inlock'] == 2) {
                            $content = "操作失败，你的账号{$userinfo['email']}已被系统锁定";
                        }else{
                            $content = "你的账号{$userinfo['email']}已经解锁";
                        }
                    }else{
                        $content = "账号未找到";
                    }
