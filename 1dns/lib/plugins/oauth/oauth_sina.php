...
class oauth_sina extends OauthBase{
	function __construct($config){
		//var_dump($config);
		//exit();
		tSession::set('apiKey',$config['apiKey']);
		tSession::set('apiSecret',$config['apiSecret']);
	}

	function getLoginUrl(){
		$o = new WeiboOAuth( tSession::get('apiKey') , tSession::get('apiSecret')  );
	...
	function getAccessToken($parms){
		$o = new WeiboOAuth( tSession::get('apiKey') , tSession::get('apiSecret') , $_SESSION['keys']['oauth_token'] , $_SESSION['keys']['oauth_token_secret']  );
		$last_key = $o->getAccessToken(  $_REQUEST['oauth_verifier'] ) ;
		$_SESSION['last_key'] = $last_key;
	}

	function getUserInfo(){
		$c  = new WeiboClient( tSession::get('apiKey') , tSession::get('apiSecret') , $_SESSION['last_key']['oauth_token'] , $_SESSION['last_key']['oauth_token_secret']  );
		$me = $c->home_timeline();
...
class WeiboOAuth {
    ...
    function getAuthorizeURL($token, $sign_in_with_Weibo = TRUE , $url) {
        ...
        if (empty($sign_in_with_Weibo)) {
            return $this->authorizeURL() . "?oauth_token={$token}&oauth_callback=" . ($url);
        } else {
            return $this->authenticateURL() . "?oauth_token={$token}&oauth_callback=". ($url);
        }
    }
