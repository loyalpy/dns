...
class pay_alipay extends paymentPlugin{
    ...
    */
    function toSubmit($payment){
    	//合作者身份(parterID)-帐号
        $merId = $this->get_conf($payment['M_Paymentid'], 'member_id');
        //交易安全校验码(key)
        $pKey = $this->get_conf($payment['M_Paymentid'], 'PrivateKey');
        $key = $pKey==''?'afsvq2mqwc7j0i69uzvukqexrzd0jq6h':$pKey;
		//订单总价
        $amount = number_format($payment['M_Amount'],2,".","");
        //商店名称
        $shopName = tUtil::filter($payment['R_Name']);
		//标题
        $subject = $shopName.'-在线充值';
		//初始化返回值
        $return = array();
        //交易接口名称
        $real_method = $this->get_conf($payment['M_Paymentid'], 'real_method');
        switch ($real_method){
            case '0':
    ...
    function callback($in,&$paymentId,&$money,&$message,&$tradeno){
        ...
        $pKey     = $this->get_conf($paymentId,'PrivateKey');
