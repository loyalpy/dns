...
class pay_wangjin extends paymentPlugin{
    ...
    */
    public function toSubmit($payment){
    	$return                = array();
    	$key                   = $this->get_conf($payment['M_Paymentid'], 'key');
    	$return['v_mid']       = $this->get_conf($payment['M_Paymentid'], 'v_mid');
    	$return['v_oid']       = $payment['M_OrderNO'];
    	$return['v_amount']    = $payment['M_Amount'];
    	$return['v_moneytype'] = "CNY";
    	$return['v_url']       = $this->callback_url;
    ...
    function callback($in,&$paymentId,&$money,&$message,&$tradeno){
        $key         = $this->get_conf($paymentId, 'key');
