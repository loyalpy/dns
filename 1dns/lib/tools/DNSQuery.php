...
class DNSQuery
{
	...
	{
		$this->server=$server?$server:(isset(App::$data['site_config']['dnsquery'])?App::$data['site_config']['dnsquery']:"8.8.8.8");
		$this->port=$port;
	...
	function ReadRecord()
	{
		...
		switch($typeid)
			...
			case "A":
				...
				$data=$ip;
				$extras['ipbin'] = $ipbin;
				$string = "主机服务记录(A): ".$domain." -------------------- ".$ip;//has IPv4 address
				break;
			...
			case "AAAA":
				...
				$data = $ip;
				$extras['ipbin'] = $ipbin;
				$string = $domain . " -------------------- " . $ip;//has IPv6 address
				break;

			case "CNAME":
				$data=$this->ReadDomainLabel();
				$string = "别名记录(CNAME): ".$domain." -------------------- ".$data;//alias of
				break;

			case "DNAME":
				$data=$this->ReadDomainLabel();
				$string=$domain." -------------------- ".$data; //ofalias of
				break;
			...
			case "MX":
				...
				$data=$this->ReadDomainLabel();
				$string = "邮件交换记录(MX): ".$domain." -------------------- ".$data." (pri=".$extras['level'].")";
				break;
				
			case "NS":
				$nameserver=$this->ReadDomainLabel();
				$data=$nameserver;
				$string = "名称服务记录(NS): ".$domain." -------------------- ".$nameserver;//nameserver
				break;
	...
	function Query($question,$type="A",$timeout=3)
	{
		$this->ClearError();
