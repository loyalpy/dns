<?php
 /**
  * @brief tHash 整合系统常用到的Hash算法及一些常用的hash算法
  * @class tHash
  * @note
  */
class tHash{
    ...
    public static function time($format='Y/m/d/His', $rand=true){
        if($rand) return tTime::get_now($format).rand(10000,99999);
        else return tTime::get_now($format);
    }
    /**
     * @brief 随机生成字符串函数
     ...
     */
    public static function md5_file($file_name){
    ...
    //创建认证URL
    public static function uri($data = array(),$pre="?"){
    	...
    	$hash = self::md5("{$encrypt_key}{$uri}{$encrypt_key}");
    	return $pre."thash={$hash}&".$uri;    	
    }
    //检查认证URL
    public static function chk_uri($hash,$data = array()){
    	$encrypt_key = App::get_conf("cfg.encrypt_key");
    	ksort($data);
    	$uri = "";
    	foreach($data as $key => $value){
    	...
    	return $hash === self::md5("{$encrypt_key}{$uri}{$encrypt_key}");    	
    }
}
?>