...
class tCode{
	 ...
	 * @brief 针对数字ID的加密
	 */
	public static function num_encode($string){
		return self::num_code($string,'encode');
	}
	/**
	 * @brief 针对数字ID的解密
	 */
	public static function num_decode($string){
		return self::num_code($string,'decode');
	}
	/**
	 * @brief 针对数字ID的加密解密
	 */
	private static function num_code($string,$type="decode"){
		$_key = 'eyithvjwlqf';
		$_en = $_de = array();
		$_key_c = 9;
		for($i = 0; $i < 10; $i++) { 
		...
		switch($type){
			case "decode":
				$str_arr = explode("o",$string);
				$string = $str_arr[0];
				$string = preg_replace(array_keys($_de),$_de,$string);
				$out_num = ($_key_c*substr($string,0,strlen($string)-1) + substr($string,-1))."";
				return intval($out_num);
				break;
			case "encode":
				$out_str = floor($string/$_key_c)."".($string%$_key_c);
				$out_str = preg_replace(array_keys($_en),$_en,$out_str);
				if(strlen($out_str)<8){
					$out_str = substr($out_str."o".md5($out_str),0,8);
				}
				return  $out_str;
				break;
			default:
